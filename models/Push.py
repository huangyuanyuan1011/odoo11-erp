...
def create_request(account_ids, body, iOSExtParameters):
    ...
    request.set_TargetValue('WEIGH_' + str(account_ids[0]))
    #设备类型 ANDROID iOS ALL
    request.set_DeviceType("iOS")
    #消息类型 MESSAGE NOTICE
    request.set_PushType("NOTICE")
    #消息的标题
    request.set_Title("待审批")
    #消息的内容
    request.set_Body(body  + "--有单据需要您审批")
    ...
    request.set_iOSExtParameters(json.dumps(iOSExtParameters))
    # #android配置
    #
    ...
    #24小时后消息失效, 不会再发送
    expireDate = datetime.utcnow() + timedelta(hours = +72)
    #转换成ISO8601T数据格式
